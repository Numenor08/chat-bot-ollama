...
const ChooseModel = () => {
    ...
    const { model, setModel, listModels, isOllamaReady } = useModelContext();
    ...
    return (
        <>
            <button
                ...
                disabled={!listModels}
                className={`${abel.className} text-nowrap text-lg text-neutral-700 dark:text-neutral-400 z-20 cursor-pointer absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-min flex bg-transparent hover:bg-gray-200 dark:hover:bg-neutral-800 hover:text-neutral-900 dark:hover:text-light overflow-y-auto overflow-x-hidden rounded-lg transition-colors duration-150`}
            >
                <div className="p-1 pl-4 pr-10">
                    {isOllamaReady ? (listModels ? model : "No Model Installed") : "Ollama is not running"}
                </div>
                <TriangleDownIcon
                    className={`w-5 h-5 absolute right-3 top-1/2 -translate-y-1/2 transition-transform duration-75 ${
                        isOpen ? "rotate-180" : ""
                    }`}
                />
            </button>
                ...
                <>
                    ...
                    >
                        {listModels?.map((model, i) => (
                            <DropdownItem
                                className="text-nowrap transition-colors duration-100 dark:hover:bg-neutral-700"
                                key={i}
                    ...
                    <div
                        className={`fixed inset-0 bg-transparent z-10 ${
                            isOpen ? "opacity-30" : "opacity-0 hidden pointer-events-none"
                        }`}
